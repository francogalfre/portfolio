export const primaryProjects = [
  {
    id: 1,
    title: "Freelanceo",
    descriptionEn:
      "An intuitive dashboard for freelancers to streamline project, client, and financial management.",
    descriptionEs:
      "Un panel intuitivo para que los freelancers gestionen proyectos, clientes y finanzas.",
    longDescriptionEn:
      "A complete dashboard for freelancers to organize clients, projects, and daily tasks, while visualizing earnings and progress through interactive charts. Designed with a modern stack using Next.js, TypeScript, Tailwind, and PostgreSQL to deliver speed and clarity.",
    longDescriptionEs:
      "Un dashboard completo para que los freelancers organicen clientes, proyectos y tareas diarias, visualizando ingresos y progreso mediante gráficos interactivos. Diseñado con Next.js, TypeScript, Tailwind y PostgreSQL para ofrecer velocidad y claridad.",
    image: "/projects/freelanceo.webp",
    image2: "/projects/freelanceo2.webp",
    href: "https://www.freelanceo.site",
    repository: "https://github.com/francogalfre/freelanceo",
    tags: ["Next.js", "Node.js", "Tailwind", "Typescript", "PostgreSQL"],
  },
  {
    id: 2,
    title: "Astrotips",
    descriptionEn:
      "A modern and fast website that provides tips, tricks, and an interactive interface for developers wanting to learn Astro.",
    descriptionEs:
      "Un sitio moderno y rápido que ofrece consejos, trucos y una interfaz interactiva para desarrolladores que quieran aprender Astro.",
    longDescriptionEn:
      "A modern platform that offers tips, best practices, and interactive examples to help developers learn Astro efficiently. Built with Astro, TypeScript, and Tailwind, it focuses on performance, clean design, and a smooth learning experience.",
    longDescriptionEs:
      "Una plataforma moderna que ofrece consejos, buenas prácticas y ejemplos interactivos para ayudar a los desarrolladores a aprender Astro de manera eficiente. Construida con Astro, TypeScript y Tailwind, se enfoca en rendimiento, diseño limpio y una experiencia de aprendizaje fluida.",
    image: "/projects/astrotips.webp",
    image2: "/projects/astrotips2.webp",
    href: "https://astrotips.vercel.app/",
    repository: "https://github.com/francogalfre/astrotips",
    tags: ["Astro", "Tailwind", "Typescript"],
  },
];

export const projects = [
  {
    id: 3,
    title: "Promptsmith | Landing Page",
    descriptionEn:
      "The promotional landing page for Promptsmith, a type-safe prompt builder for the Vercel AI SDK, communicating its features and developer experience.",
    descriptionEs:
      "Landing page para Promptsmith, un constructor de prompts type-safe para el Vercel AI SDK, enfocada en comunicar sus características clave.",
    image: "/projects/promptsmith.webp",
    href: "https://promptsmith.galfrevn.com/",
    repository: "https://github.com/galfrevn/promptsmith",
    tags: ["Next.js", "Tailwind", "Typescript", "Motion"],
  },
  {
    id: 4,
    title: "Patitas",
    descriptionEn:
      "A platform that connects pets with adoptive families, allowing users to publish or search for pets available for adoption through an intuitive and modern interface.",
    descriptionEs:
      "Una plataforma que conecta mascotas con familias adoptivas, permitiendo a los usuarios publicar o buscar mascotas disponibles para adopción.",
    image: "/projects/patitas.webp",
    href: "https://www.patitas.site/",
    repository: "https://github.com/francogalfre/patitas",
    tags: ["Next.js", "Node.js", "Tailwind", "Typescript", "PostgreSQL"],
  },
  {
    id: 5,
    title: "Github Profile Explorer",
    descriptionEn:
      "A simple and fast tool to explore GitHub profiles, showing user details, repositories, and activity with a clean interface and dark/light themes.",
    descriptionEs:
      "Una herramienta simple y rápida para explorar perfiles de GitHub, mostrando detalles del usuario, repositorios y actividad con una interfaz limpia y modos oscuro/claro.",
    image: "/projects/github-profile.webp",
    href: "https://github-profiles-explorer.vercel.app/",
    repository: "https://github.com/francogalfre/github-profile-explorer",
    tags: ["React", "Javascript", "CSS", "Github API"],
  },
];
